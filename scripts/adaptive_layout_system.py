...
# 示例场景：超长文本自适应演示
class LongTextAdaptiveDemo(Scene):
    def construct(self):
        # 设置抖音竖屏比例并创建安全区域
        ...
        # 创建标题
        title = Text("超长文本自适应演示", font_size=48, color=BLUE)
...
在高等数学中，几何思想继续发挥重要作用，如在微积分、线性代数、复分析等领域。例如，导数可以理解为曲线的切线斜率，积分可以理解为曲线下的面积。

计算机图形学大量应用几何学原理，如三维建模、光线追踪、碰撞检测等。游戏开发、动画制作、虚拟现实等领域都依赖于几何算法。

人工智能和机器学习也使用几何概念，如在流形学习、聚类分析、降维等技术中。几何直觉帮助我们理解高维数据的结构和关系。

总之，几何学是连接抽象数学与现实世界的桥梁，通过可视化和直观理解，使复杂的数学概念变得更加易于理解和应用。
"""
        
        # 方法1：自动分页显示超长文本
        self.play(FadeOut(title))
        subtitle = Text("方法1：自动分页显示", font_size=36, color=GREEN)
        subtitle.to_edge(UP, buff=1)
        ...
        page_counter.to_edge(DOWN, buff=0.5)
        ...
        for i, page in enumerate(pages):
            ...
            page.next_to(subtitle, DOWN, buff=0.8)
        ...
        # 清除页面
        self.play(FadeOut(current_page), FadeOut(page_counter), FadeOut(subtitle))
        
        # 方法2：滚动显示超长文本
        subtitle = Text("方法2：滚动显示文本", font_size=36, color=BLUE)
        subtitle.to_edge(UP, buff=1)
        self.play(Write(subtitle))
        ...
        # 设置初始位置（底部之下）
        scrolling_text.next_to(subtitle, DOWN, buff=0.8)
        original_position = scrolling_text.get_center()
        
        # 如果文本太长，设置一个裁剪区域
        max_visible_height = safe_height * 0.75
        if scrolling_text.height > max_visible_height:
            # 创建裁剪区域
            clip_rect = Rectangle(
                width=safe_width,
            ...
            clip_rect.move_to(scrolling_text.get_top() + DOWN * max_visible_height/2)
            
            # 显示文本
            self.play(FadeIn(scrolling_text))
            
            # 计算需要滚动的距离
            scroll_distance = scrolling_text.height - max_visible_height
            
            # 滚动动画
            self.play(
                scrolling_text.animate.shift(UP * scroll_distance),
                run_time=8,
                rate_func=linear
            )
            self.wait(1)
            ...
            # 滚回原位
            self.play(
                scrolling_text.animate.move_to(original_position),
                run_time=1
            )
        else:
            # 如果文本不太长，直接显示
        ...
        # 清除滚动文本
        self.play(FadeOut(scrolling_text), FadeOut(subtitle))
        
        # 方法3：高亮关键词
        subtitle = Text("方法3：高亮关键词", font_size=36, color=YELLOW)
        subtitle.to_edge(UP, buff=1)
        self.play(Write(subtitle))
        
        # 创建适合屏幕的文本（取前几段）
        highlight_text_content = "\n\n".join(long_text.strip().split("\n\n")[:3])
        ...
        highlight_text.next_to(subtitle, DOWN, buff=0.8)
        
        self.play(FadeIn(highlight_text))
        self.wait(1)
        
        # 高亮关键词
        keywords = ["欧几里得几何", "解析几何", "微分几何", "勾股定理", "动画可视化"]
        AdaptiveLayoutSystem.highlight_text_parts(
            self, highlight_text, keywords, color=YELLOW, animation_per_part=True, run_time=0.7
        )
        
        self.wait(1)
        
        # 清除高亮文本
        self.play(FadeOut(highlight_text), FadeOut(subtitle))
        
        # 添加平台元素
        AdaptiveLayoutSystem.add_platform_elements(self, platform="tiktok")
        
        # 结束
        final_text = Text("自适应布局系统演示完成", font_size=42, color=GREEN)
...
# 运行命令：
# manim -pql adaptive_layout_system.py LongTextAdaptiveDemo